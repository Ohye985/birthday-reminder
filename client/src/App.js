...
export default function App() {
  ...
  async function onSubmit(e) {
    ...
    try {
      const res = await createUser({ username, email, dob });
      if (res && res.error) {
    ...
    <div style={{ maxWidth: 900, margin: '24px auto', fontFamily: 'Arial, sans-serif', padding: 12 }}>
      ...
      <form onSubmit={onSubmit} style={{ display: 'grid', gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button type="button" onClick={() => { setUsername(''); setEmail(''); setDob(''); }}>Clear</button>
      ...
      <footer style={{ marginTop: 20, fontSize: 13, color: '#ccc' }}>
        Tip: Use a Gmail App Password for GMAIL_PASS in the server .env (recommended if you have 2FA).
      </footer>
    </div>
  );
}
